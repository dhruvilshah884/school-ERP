...
export default function StudentPage(){
    ...
    const { data: studentList, refetch } = useQuery(
      ...
      const { mutate: handleDelete } = useMutation(
        (studentId: string) => deleteStudent(schoolId as string, studentId),
        {
          onSuccess: data => {
            toast({
              variant: 'destructive',
                      ...
                      <TableBody className='divide-y divide-gray-100 dark:divide-white/[0.05]'>
                        {studentData.map((item: any) => (
                          <TableRow key={item._id} className='border-b last:border-b-0'>
                            ...
                            <TableCell className='py-2 px-5 text-sm '>{item.class_id?.name}/{item?.division}</TableCell>
                            ...
                            <TableCell className='py-2 text-sm '>
                              <div className='flex space-x-1 justify-end '>
                                ...
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      <AlertDialogTitle>Are you sure you want to delete {item.user_id?.name}?</AlertDialogTitle>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                      <AlertDialogCancel
                                        onClick={() => {
